...
void Nokia5110_SetLine(uint32_t i1,uint32_t i2, uint32_t j1,uint32_t j2){
    int n_pontos=0;
    uint32_t iponto,jponto;
    n_pontos = abs((int)(i2-i1))>abs((int)(j2-j1))?abs((int)(i2-i1))-1:abs((int)(j2-j1))-1;
    for(int i=1;i<=n_pontos;i++){
        iponto=i1 + (uint32_t)round((int)(i2-i1)*((float)i/(float)n_pontos));
        jponto=j1 + (uint32_t)round((int)(j2-j1)*((float)i/(float)n_pontos));

        Screen[84*(iponto>>3) + jponto] |= Masks[iponto&0x07];
    }
